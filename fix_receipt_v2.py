#!/usr/bin/env python3
"""
Manual fix for the receipt generation issue.

This script will:
1. Locate the pos_page.py file
2. Back up the original file
3. Replace the print_sale_ticket method with a new implementation using reportlab
...
def replace_function_in_file():
    """Replace the print_sale_ticket function in the pos_page.py file."""
    # Find the pos_page.py file path
    pos_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "desktop_app", "src", "ui", "main_window_new", "pos_page.py"
    )
    
    print(f"Target file path: {pos_file_path}")
    
    if not os.path.exists(pos_file_path):
        print(f"❌ Error: File not found at {pos_file_path}")
        return False
        
    # Create backup filename with timestamp
    backup_suffix = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = f"{pos_file_path}.bak_{backup_suffix}"
    ...
    # Check if imports exist and add them if not
    ...
    if not match:
        print("❌ Could not find the print_sale_ticket method in the file.")
        return False
...
def main():
    """Main function."""
    print("===== MANUAL FIX FOR RECEIPT GENERATION =====")

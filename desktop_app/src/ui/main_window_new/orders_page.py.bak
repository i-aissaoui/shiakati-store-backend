...
class OrdersPageMixin:
    ...
    def generate_monthly_invoice(self):
        ...
        try:
            ...
            try:
                # Fetch orders using the improved date range method with fallback
                ...
                if not orders:
                    ...
                    QMessageBox.warning(
                        self, 
                        "No Orders", 
                        f"No orders found for {month_names[month_idx-1]} {year}.\nDate range: {start_date_str} to {end_date_str}"
                    )
                    return
                    
                # Debug: Show the first few orders dates for verification
                if orders:                    for i, order in enumerate(orders[:3]):
                        if len(orders) > 3:
                        except Exception as e:
                progress.close()
